...
window.U = (() => {
  ...
  // Chinese-adapted canvas/SVG font strings (must match css/style.css stacks exactly)
  const FONT = {
    serif: '"et-book","Songti SC","Noto Serif CJK SC","Source Han Serif SC",serif',
    mono: 'Menlo,"PingFang SC","Microsoft YaHei",monospace',
  };
  ...
  function showDrill({ title, value, delta, sub, source, x, y }) {
    drill.innerHTML = `<button class="d-close">✕</button>
      ...
      ${source ? `<div class="d-src">来源 · ${source}</div>` : ""}`;
  ...
  function frame(el, { title, sub, src }) {
    ...
    if (src) {
      ...
      s.className = "chart-src"; s.textContent = "来源 · " + src;
      el.appendChild(s);
    }
    return body;
  }

  // Bind a drill-down click to any element (canvas hot-zone or DOM node)
  function drillable(el, info) {
    el.style.cursor = "pointer";
    el.setAttribute && el.setAttribute("data-drill-keep", "1");
    el.addEventListener("click", e => {
      e.stopPropagation();
      showDrill({ ...info, x: e.clientX, y: e.clientY });
    });
  }

  return { TAU, clamp, lerp, smooth, ease, makeRng, PAL, SERIES, FONT, REDUCE, bindCanvas, project, dotField, countUp, fmt, showDrill, hideDrill, showTip, hideTip, frame, drillable };
})();
